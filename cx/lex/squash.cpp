...
PRIVATE void col_cpy(int *dest, int *src, int nrows, int n_src_cols,
                     int n_dest_cols) {
...
PRIVATE void reduce(ROW *dtran, int *p_nrows, int *p_ncols) {
  /* dtran: DFA transition table
   * p_nrows: # of states in dtran; p_ncols: Pointer to column count
   */
  ...
  for (r_ncols = 0;; r_ncols++) {
     ...
     */
    for (r_ncols = 0; Col_map[i] != -1 && i < ncols; i++)
      ;
...
PRIVATE void print_col_map(FILE *fp) {
  static char *text[] = {
...
PRIVATE void print_row_map(fp, nrows) FILE *fp;
int nrows;
{
...
PRIVATE void pmap(fp, p, n) FILE *fp; /* output stream    */
...
/*----------------------------------------------------------------------*/
PUBLIC void cnext(fp, name) FILE *fp;
char *name;
{
